...
# Let the user enter the model and some params:
method = input("Enter a number to choose the method: ")
...
n_step = 1
if method == 3:
    n_step = input("Select n for n-step model-based method (min 1, max 5): ")
    while n_step not in valid_inputs:
        n_step = input("Invalid input, please re-enter: ")
    
    n_step = int(n_step)

LoCA_pretraining = input("Select whether to conduct LoCA pretraining (t/f): ")
valid_inputs = ['T', 'F', 't', 'f', 'true', 'false', 'True', 'False', '1', '0']
while LoCA_pretraining not in valid_inputs:
    LoCA_pretraining = input("Invalid input, please re-enter: ")

if LoCA_pretraining in ['T', 't', 'true', 'True', '1']:
    LoCA_pretraining = True
else:
    LoCA_pretraining = False

# Non-user parameter settings:
alpha_multp = 1.0          # any value > 0 - tweak step-size hyperparameter
S_multp = 1                # {1, 2, 3, ... } - optionally enlarge the state space
